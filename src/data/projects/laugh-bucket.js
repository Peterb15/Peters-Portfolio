const project = {
  "id": "laugh-bucket",
  "title": "The Laugh Bucket",
  "tags": ["installation", "alt-arcade", "fabrication", "interactive"],
  "tagline": "An interactive audio installation for spontaneous sound layering and feedback",
  "role": "Lead Designer & Developer",
  "tags": "Arduino, NDI, Audacity, Fabrication",
  "date": "Nov. 2024",
  "heroMedia": "/assets/laugh-bucket/fullDisplay.jpg",
  "overview": "An interactive installation that let users remix laughter through Arduino-triggered recording and playback, layered using NDI systems. Exhibited at the Whaaat!? Festival 2024 at CU Boulder's ATLAS Institute. The Laugh Bucket invites spontaneous play, allowing users to contribute laughter and hear it transformed into layered sonic chaos.",
  "myRole": [
    "Integrated Arduino + NDI systems for real-time audio",
    "Fabricated installation housing and user interaction elements",
    "Directed installation presentation at the Whaaat!? Festival"
  ],
  "processSections": [
    {
      "title": "Concept & Ideation",
      "content": "Explored chaotic sound play through public interaction—remixing laughter as both a personal and shared experience. The goal was to create something strange and attention-grabbing; something that left people confused, yet intrigued. Because it required multiple laughs to activate, many users simply laughed into a microphone and walked away—contributing unknowingly to future outputs."
    },
    {
      "title": "Technical Build",
      "content": "Used Arduino inputs to trigger and layer audio samples through NDI. Each new input was remixed over previous recordings, with visual feedback included."
    },
    {
      "title": "Exhibition & Feedback",
      "content": "One of the more interesting outcomes was how interaction with the Laugh Bucket evolved over the course of the festival. As attendees began to understand its purpose, many returned to record multiple laughs—using them later to surprise unsuspecting visitors. It created a playful feedback loop between users."
    }
  ],
  "gallery": [
    {
      "type": "video",
      "src": "https://www.youtube.com/embed/H6jwWfM1DRg?si=MOH6X25X0Y4bXrEM",
      "title": "Laugh Bucket Demo Video"
    },
    {
      "type": "image",
      "src": "/assets/laugh-bucket/Image1.jpg",
      "alt": "Laugh Bucket wiring",
      "caption": "Internal audio routing with Arduino & NDI"
    },
    {
      "type": "image",
      "src": "/assets/laugh-bucket/Image 2.jpg",
      "alt": "Exhibit side view",
      "caption": "Enclosure design from side angle"
    },
    {
      "type": "image",
      "src": "/assets/laugh-bucket/image3.jpg",
      "alt": "Laugh Bucket at Festival",
      "caption": "Patrons using the Laugh Bucket during Whaaat!? Festival demo"
    }
  ],
  "reflection": "This project deepened my interest in physical computing as a medium for shared joy. I became fascinated by how interactions developed and evolved throughout the festival. It also challenged my fabrication skills to build enclosures that were not only functional, but aesthetically pleasing.",
  "collaborators": [
    {
      "name": "Josh Huges",
      "role": "Audio Engineer"
    }
  ],
  "externalLinks": [
    {
      "href": "https://www.youtube.com/embed/H6jwWfM1DRg?si=MOH6X25X0Y4bXrEM",
      "label": "Watch Project Video"
    }
  ]
};

export default project;
